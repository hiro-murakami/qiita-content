...
tags:
  ...
  - ガントチャート
private: false
updated_at: '2026-08-15T08:10:00+09:00'
...
- **ズーム**、**クリティカルパス表示**、**行マーカー**、**画像添付**、**一般公開URL**など、v0.9.0 で機能をさらに拡充
...
マイルストーンがプロジェクト全体の節目であるのに対し、**マーカー**は各行（タスクグループ）のタイムライン上に配置する目印です。

- 三角形アイコン＋ラベルで、行ごとの重要な日時をマーク
- **3種類のスタイル**: ◀（左向き） / ▶（右向き） / ▼（下向き）
- **5段階のフォントサイズ**: 極小 / 小 / 中 / 大 / 特大
- 16色のプリセットカラー＋カスタム色指定
- ダブルクリックで編集、右クリックで編集・削除

「このタスクグループのレビュー期限」「チームごとのマイルストーン」など、行レベルで細かい目印を置きたい場合に活用できます。

### 4.6. 🔍 ズーム ─ 見たい範囲にフォーカス

**Ctrl（Cmd）+ マウスホイール**で、チャートをズームイン・ズームアウトできます。

- 表示設定メニューのスライダーからも調整可能
- 表示モード（時間/日/月）ごとに独立したズームレベルを管理

タスクが密集しているエリアを拡大して確認したり、全体を俯瞰したりと、状況に応じた表示が可能です。
...
- 接続線を右クリックすると「接続線を削除」メニューが表示され、不要な依存関係をすぐに解除できます
...
「プロジェクト全体のボトルネックはどこか？」「どのタスクの遅れが全体に影響するか？」を一目で把握できます。
...
- **フリーワード検索**: タスク名にマッチするバーがハイライト表示。「行も検索」チェックボックスをONにすると、行名も検索対象になります
...
- **3つのアップロード方法**: ファイル選択 / ドラッグ＆ドロップ / クリップボードから貼り付け（Ctrl+V / ⌘+V）
- **自動圧縮**: 5MBを超える画像は自動的にリサイズ・圧縮されます
- 添付した画像はサムネイル一覧で表示され、不要なものはワンクリックで削除可能

スクリーンショットを貼り付けて仕様の補足にしたり、デザインカンプを添付して認識を合わせたりと、テキストだけでは伝えにくい情報を共有できます。

### 🌐 一般公開モード

プロジェクト設定で「一般公開」をONにすると、**ログインなしでURLを知っている人なら誰でも閲覧**できるようになります。

- 公開されたプロジェクトは読み取り専用で表示されます
- テーマ（ライト/ダーク/システム）の切り替えは公開閲覧時でも可能

社外へのスケジュール共有や、ポートフォリオ的な公開にも活用できます。

### 🔑 APIキー管理

プロジェクトデータを外部ツールやCI/CDパイプラインから操作するための**APIキー**を発行・管理できます。

- **スコープ選択**: 読み取り専用（read）/ 読み書き（read-write）
- キーの作成・一覧・無効化をGUI上で完結
- 作成時に表示されるキーは**一度だけ**表示されるセキュアな設計
...
また、プロジェクトを**一般公開**に設定すると、ログインなしでもURLを共有するだけで閲覧可能になります（編集はできません）。
...
| バックエンド | Firebase Cloud Functions + Prisma + MariaDB |
| リアルタイム | Firestore (プレゼンス・編集イベント) |
| ホスティング | Firebase Hosting |

ガントチャートの描画エンジン（[`@mogura/moguchart-core`](https://github.com/hiro-murakami/moguchart-core)）はフレームワーク非依存の Web Components として別途開発・[npm公開](https://www.npmjs.com/package/@mogura/moguchart-core)しています。[オンラインデモ](https://moguchart-core.vercel.app/)もあります。Vue だけでなく React や Angular でも使えるので、「自分のアプリにガントチャートを組み込みたい」という方にも活用いただけます。
