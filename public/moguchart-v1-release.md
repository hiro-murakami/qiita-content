...
## 🚀 【Update】さらに進化した v1.1.0 の改善点

正式リリース後もユーザー体験の向上を続け、最新の **v1.1.0** では以下のブラッシュアップを実施しました！

1. **公式 Vue 3 ラッパー導入 ＆ プラグイン遅延読み込みによるさらなるサクサク動作**:
...
2. **画像プレビューポップアップの快適化（ホバーディレイ ＆ 排他制御）**:
   - サムネイル通過時のちらつきを防ぐため 500ms のホバーディレイを導入し、クリック時には即座にキャンセル。ガントチャートの標準ツールチップとの競合・重なりも解消しました
3. **エクスポート時の表示倍率（ズーム）自動正規化**:
...
構想から開発を重ね、実務で自信を持って使える **v1.0.0**、そしてさらに使いやすさと品質を極めた **v1.1.0** へと進化を遂げました。
