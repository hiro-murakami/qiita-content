...
tags:
  ...
  - ガントチャート
private: false
updated_at: '2026-09-05T07:55:55+09:00'
...
| 📤 **多彩なエクスポート** | PDF（複数ページ分割対応） / PNG / CSV / Excel / **ZIP（添付画像完全同梱・自動復元）** |
...
flowchart TB
    subgraph Client ["クライアント (Browser)"]
        ...
        Core["@mogura/moguchart-core<br>(Lit Web Component)<br>・仮想スクロール<br>・D&D / 行間移動<br>・矩形範囲選択 (ラバーバンド)<br>・進捗ドラッグ編集<br>・ミニマップ鳥瞰描画<br>・クリティカルパス計算<br>・ホイールズーム"]
...
| 技術 | バージョン | 用途 |
|---|---|---|
| **@mogura/moguchart-core** | ^0.12.0 | Lit ベースのガントチャート描画エンジン |
...
ルートの `package.json` のバージョン（例: `0.12.0`）を、フロント・バックエンド・ドキュメントへビルド前に自動同期します。
