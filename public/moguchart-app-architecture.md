...
| **Chrome DevTools Protocol (CDP)** | リモートデバッグ・自動画面検証ツール (`scripts/debug-chrome.mjs`) |
...
コアライブラリ（`@mogura/moguchart-core` v1.2.0）から発行される `command` および `history-change` イベントをハンドリングし、タスクのドラッグ移動（単一・矩形選択による複数一括移動）、ドラッグリサイズ、タスクバー上の進捗ハンドル操作などの操作履歴をアプリケーション側の Undo / Redo スタックへシームレスに同期しています。

- **広範な操作の取り消し・やり直し**: タスク作成・編集・削除、クリップボード貼り付け、行の追加・編集・削除・並び替え・インデント/アウトデント、親行の一括開閉、依存関係の作成・削除など、チャート上のあらゆる変更操作を `Cmd+Z` / `Ctrl+Z`（元に戻す）および `Cmd+Shift+Z` / `Ctrl+Y`（やり直し）で安全に取り消し・再実行できます
- **共同編集との安全な調和**: Undo / Redo 実行時には Firestore 経由で他クライアントへ即座に変更通知（`full_reload`）を発行し、画面間の不整合が起きない安全機構を敷いています

### 11. 依存関係線の削除UI強化 ＆ 削除確認ダイアログ

タスク間の接続線（矢印）を左クリックして選択・削除するコア機能（`@dependency-delete` イベント）に追従したほか、右クリックコンテキストメニューからの削除にも対応しました。

接続線の削除を実行する際、先行タスク名と後続タスク名を明示した確認ダイアログ（「『タスクA』から『タスクB』への接続線を削除してもよろしいですか？」）を表示し、意図しない誤削除を確実に防止。万が一削除した場合でも、Undo（`Cmd+Z`）により直ちに復元可能です。

### 12. ズーム管理とスケール設定のコアチャート移譲

全体の表示倍率（50%〜200%）、基準カレンダー幅、行ヘッダー幅、フォントスケール（`--moguchart-font-scale`）の管理をコアチャート（`chartOption.zoom`）へ完全に委譲しました。
...
### 6. Chrome リモートデバッグツールによる自動検証環境 (`scripts/debug-chrome.mjs`)

macOS / Windows / Linux の各プラットフォームに対応し、Chrome をリモートデバッグポート（`9222`）でバックグラウンド起動する `pnpm run chrome:debug` コマンドを追加しました。

CDP (Chrome DevTools Protocol) 連携スクリプト（`scripts/debug-chrome.mjs`）を整備し、タブ一覧取得、新規ページオープン、画面キャプチャ撮影、JavaScript 評価（eval）によるブラウザ自動検証やテストを容易にしています。
