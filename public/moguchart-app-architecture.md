...
│                       クライアント                            │
│  ┌──────────────────────────────────────────────────────┐   │
│  │           Vue 3 + Vuetify 4 (Frontend)               │   │
│  │  ┌──────────┐ ┌──────────┐ ┌───────────────────┐    │   │
│  │  │  Pinia   │ │  Router  │ │  Custom Components │    │   │
│  │  │ (Store)  │ │          │ │  (34+ダイアログ)    │    │   │
│  │  └──────────┘ └──────────┘ └───────────────────┘    │   │
│  │                      │                               │   │
│  │           ┌──────────▼──────────┐                    │   │
...
└─────────────────────────────────────────────────────────────┘
                           │
              Firebase SDK │ Cloud Functions API
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                        Firebase                              │
│  ┌─────────────┐ ┌──────────────┐ ┌──────────────────┐     │
│  │  Hosting    │ │   Functions  │ │   Firestore      │     │
│  │  (SPA配信)  │ │  (API Server)│ │ (プレゼンス/イベント)│     │
│  └─────────────┘ └──────┬───────┘ └──────────────────┘     │
│                         │                                    │
│  ┌─────────────┐  ┌─────▼────────┐  ┌──────────────────┐   │
│  │  Auth       │  │   Prisma ORM │  │   Storage        │   │
│  │  (認証)     │  │              │  │  (ファイル管理)   │   │
│  └─────────────┘  └─────┬────────┘  └──────────────────┘   │
│                         │                                    │
│                   ┌─────▼────────┐                           │
...
約4ヶ月間で以下の機能を追加してきました：

- **v0.5.0**: スナップショット、バックアップ復元、チュートリアル
- **v0.5.1**: タスクテンプレート、ラベルフィルタ
- **v0.5.2**: タスクロック、CSV/Excel エクスポート強化
- **v0.5.3**: 自動スナップショット、コメントサイドバー
- **v0.5.4**: プロジェクト複製強化、マイルストーンハイライト
- **v0.5.5**: PDFエクスポート、依存関係線、リリースノート表示
- **v0.5.6**: テーマ統合、バー高さ追加、読み取り専用モード
- **v0.6.0**: 時間単位/月単位モード、行ラベル、カラーパレット名
- **v0.6.1**: パターン設定、枠線設定、バー影設定
- **v0.6.2**: Mac Ctrl+ドラッグ修正、コピー時の依存関係除外
- **v0.6.3**: ゲストログイン
- **v0.6.4**: 期間スライド、スナップ単位設定追加
